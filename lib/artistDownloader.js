...
import { ytdl } from './functions.js';
import { logDownload } from './logging.js';
 ...
 */
export async function downloadArtistMedia({ sock, msg, args, commandName, downloadingState, format }) {
  ...
  try {
    ...
    const items = searchResults.videos.slice(0, 5); // Limitar a 5 para no hacer spam
    ...
    for (const item of items) {
      try {
        ...
        const downloadType = format === 'audio' ? 'mp3' : 'mp4';

        const ytdlResult = await ytdl(url, downloadType);
        const mediaResponse = await axios.get(ytdlResult.url, { responseType: 'arraybuffer' });
        const mediaBuffer = mediaResponse.data;

        if (mediaBuffer && mediaBuffer.length > 0) {
          let sentMsg;
          if (format === 'audio') {
            sentMsg = await sock.sendMessage(msg.key.remoteJid, { audio: mediaBuffer, mimetype: 'audio/mpeg' }, { quoted: msg });
          } else {
            sentMsg = await sock.sendMessage(msg.key.remoteJid, { video: mediaBuffer, mimetype: 'video/mp4', caption: title }, { quoted: msg });
          }
        ...
        await sock.sendMessage(msg.key.remoteJid, { text: `❌ Falló la descarga de *${item.title}*. Continuando...` }, { quoted: msg });
      }
    ...
    user.coins += COST;
    writeUsersDb(usersDb);
    const errorText = `❌ *Error en ${commandName}:* El servicio de búsqueda o descarga parece no estar disponible.\n\nSe te han devuelto tus ${COST} coins.`;
    if (waitingMsg) {
